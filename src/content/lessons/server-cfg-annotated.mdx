...
<StepGroup>
  ...
  <Step num={2} title="Write the complete file, top to bottom" outcome="You have a full, bootable, 2026-correct server.cfg.">
    ...
    ensure es_extended    # framework, needs oxmysql + ox_lib above it
    ensure my_job_script  # your script, needs es_extended above it
    ...
    sv_enforceGameBuild 3407       # Agents of Sabotage; use 3751 for the newest
  ...
  <Step num={3} title="Replace the two values that must be yours" outcome="The file is no longer a template; it is your server.">
    ...
    Replace `changeme` with the real key from `https://portal.cfx.re`. Replace `identifier.fivem:1` with your own identifier. To find it, boot the server, join it, and run `status` in the console to print every connected player with their identifiers. Copy your `license:` or `fivem:` value and paste it in.
  ...
  <Step num={4} title="Boot the server and read the console" outcome="The server starts clean and lands on your config.">
    Launch the server from inside the `server-data` folder so it finds `server.cfg`:
...
<FailureList
  items={[
    ...
    { symptom: "sv_enforceGameBuild change is ignored until you restart", fix: "That convar is startup-only and cannot change at runtime. Set it in server.cfg, then fully restart the server. Pick a current build like 3407 or 3751, not an outdated one." },
