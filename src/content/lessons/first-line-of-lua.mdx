...
`print` is a **function**: a named action you can call. You call it by writing its name, then a pair of parentheses, and inside the parentheses you put what you want it to act on. Here you hand it one string. The value you hand a function is called an **argument**. So in plain words this line says, take this string and write it to the console. That is the entire job of `print`. It does not pop up on a player's screen and it does not appear in chat. It writes to the console, which on the server is the txAdmin Live Console.
