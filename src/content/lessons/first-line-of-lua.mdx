...
<FailureList
  items={[
    ...
    { symptom: "The command does nothing when a player runs it in chat", fix: "The restricted flag is true, so only the server console (and aces) may run it. Run first_line_of_lua from the txAdmin Live Console, not the F8 chat box." },
  ]}
/>
