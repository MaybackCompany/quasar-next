<LessonHero
  ...
  meta={[
    { label: "You'll build", value: "A qu_variables_numbers_strings resource that prints a structured server info line: name, slots, version." },
    { label: "Time", value: "~15 minutes" },
    { label: "You need", value: <>The <code>qu_first_line_of_lua</code> resource you built in <a href="/lessons/first-line-of-lua">Your first line of Lua</a>; we pick up where it stopped.</> },
...
<Prereq
  items={[
    "I built qu_first_line_of_lua in the first Lua lesson and saw it print.",
...
resources/qu_variables_numbers_strings/
  ...
  <Step num={3} title="Write fxmanifest.lua" outcome="FiveM knows which files to load.">
    ...
    Same manifest shape as [your first Lua resource](/lessons/first-line-of-lua). There is no `-- Lua 5.4 is the default runtime in 2026; the lua54 directive is no longer needed` line: that setting was deprecated in June 2025 and is now ignored, because Lua 5.4 is the only Lua runtime FiveM ships. Leave it out.
  </Step>
  ...
  <Step num={5} title="Start it and read the proof" outcome="The expected proof appears in the txAdmin Live Console.">
    ...
    Unlike [your first Lua lesson](/lessons/first-line-of-lua), there is no command to type. These three prints sit at the top level of `server.lua`, so they fire the instant the resource starts. The restart **is** the test. Read the three lines it prints:
