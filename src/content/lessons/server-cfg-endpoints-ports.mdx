<LessonHero
  ...
  meta={[
    { label: "You'll build", value: "A server.cfg network block (endpoint_add_udp, endpoint_add_tcp, sv_endpointPrivacy, sv_maxClients) plus a friend-connects-by-IP test" },
    ...
    { label: "You'll learn", value: "endpoint_add_udp/tcp 0.0.0.0:30120 -> why 30120 is TCP and UDP -> sv_endpointPrivacy -> sv_maxClients -> port-forwarding on a home router -> firewall allow -> verifying with a real connection" },
  ]}
>
...
An endpoint is an address plus a port that the server promises to listen on. `endpoint_add_udp` and `endpoint_add_tcp` each open a listening socket. The string has two halves split by the colon.
...
These do not open ports; they set policy on the door you already opened.
...
<FailureList
  items={[
    { symptom: "You can join with connect 127.0.0.1 but no one outside can, even on the same LAN", fix: "Your endpoint is bound to localhost. Change endpoint_add_udp/tcp to 0.0.0.0:30120 (not 127.0.0.1:30120), restart, then confirm by joining from another machine on your LAN using the host's private IP." },
    ...
    { symptom: "Console keeps logging 'Could not contact the server browser' and the server vanishes from the list", fix: "If you run DDoS protection or a strict firewall, it is blocking Cfx.re's polling IPs. Allow every address listed at https://static.cfx.re/ips.txt. For a purely local dev server, add sv_master1 \"\" to make it private so the browser shows no join button (it still posts a heartbeat on startup)." },
  ]}
/>
...
<Exercise>
  Right now your server listens on the default port 30120. Prove you understand the endpoint and forwarding chain by moving the whole server to a second port, end to end. Change both endpoint lines to 30125, keeping UDP above TCP:
...
<Refs
  items={[
    ...
    { label: "FiveM proxy and endpoint privacy", href: "https://docs.fivem.net/docs/server-manual/proxy-setup/" },
