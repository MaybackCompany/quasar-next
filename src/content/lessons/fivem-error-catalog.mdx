<LessonHero
 ...
 { label: "You'll learn", value: "How to read a traceback (resource, file, line). Then the eight errors that stop every beginner, with the exact text, the meaning, and the fix for each." },
 ]}
>
 Errors are not the enemy. A red line in the console is the server telling you exactly what is wrong and exactly where. The only skill you are missing is reading it. In this lesson you learn to read an error like an address. Then you get a catalog of the eight errors every FiveM beginner hits. Each one has its exact console text, what it actually means, and the single fix that clears it.
</LessonHero>
...
<Callout variant="fivem" title="Read the error first, change code second">
 Beginners delete code and guess. Professionals read the error, find the named resource, file, and line, and fix that one spot. This lesson trains the reading move first. Every fix in the catalog starts with knowing what the console already told you.
 </Callout>

 <ExpectedOutput>
 You can open any FiveM error, point at the resource name, file path, and line number, and say "this crashed because X, the fix is Y." You have a printed or saved copy of this catalog you can Ctrl-F whenever the console turns red.
 </ExpectedOutput>
...
That is the whole skill. Resource, file, line, meaning. The catalog below gives you the meaning and the fix for the eight you will meet first. You find every one of them the same way. Read the brackets, open the file, jump to the line.
...
Below are the eight errors that stop beginners, in the order you tend to meet them. First the manifest problems that stop a resource loading, then runtime nil-value errors, then dependency, database, and config problems. Each one gives you the exact console text, what it means, and the fix.
...
Two root causes sit behind this message. The first is a manifest that does not parse. The most common version is the singular versus plural directive trap. The singular `client_script` and `server_script` take exactly **one** filename as a string. The plural `client_scripts` and `server_scripts` take a Lua **table** in curly braces. Mix them up and the manifest silently drops files or refuses to load.
...
That is why `ensure` and order in `server.cfg` matter so much. Databases and libraries first, frameworks next, your own resources last. Get the order right and a surprising number of mysterious errors never appear.
...
### Reading the colour, not the words
...
<FailureList
 ...
 { symptom: "Unable to establish a connection to the database", fix: "This is config, not code. Check mysql_connection_string in server.cfg, start the MariaDB service (services.msc, or Get-Service MariaDB*), and confirm the database name exists. For a port 3306 conflict, stop the conflicting service. If the password contains reserved characters like ;, / ? : @ & = + $ #, switch from the URI form to the key=value form (host=...;user=...;password=...;database=...)." },
 { symptom: "ER_NO_SUCH_TABLE / no such table", fix: "The connection works but the table was never created. Import the script's provided .sql file into your database once, then confirm the table appears in HeidiSQL or DBeaver." },
 ...
 { symptom: "The error message is long and confusing", fix: "Read ONLY the first red line. It names the resource, file, and line. Ignore the stack of file paths below it; for a beginner the top line is the one that failed." },
 { symptom: "I fixed the error but it came back", fix: "You fixed a symptom, not the cause. Read the error again. If it says 'nil value', find where that variable was supposed to be set and make sure that ran first." },
 { symptom: "The error mentions a file I never created", fix: "Check whether the resource name in brackets is one of yours. If it is not, the bug is in a dependency you installed, not in your code." },
 { symptom: "No error in console but the script still does nothing", fix: "This is a logic error: the code is valid but does the wrong thing. Add a print('DEBUG: made it here') line. If it never prints, the code before it never ran." },
