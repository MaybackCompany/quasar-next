...
The name in parentheses, here `QBCore`, is the thing that was `nil`. The classic cause is using a framework object before it has loaded, or before its resource has started. You wrote `QBCore.Functions...` but `QBCore` itself was never filled in, so reaching into it with the dot fails.

```lua
-- Error: attempt to index a nil value (global 'QBCore')
-- QBCore is nil because qb-core had not started, so the export returned nothing.
local QBCore = exports['qb-core']:GetCoreObject()
local money = QBCore.Functions.GetPlayerData().money  -- indexing nil here

-- FIX: ensure qb-core starts BEFORE this resource in server.cfg,
-- and guard before you index, so a not-yet-loaded object cannot crash you.
...
The fix is two parts. First, make sure the resource that provides the value is started before yours (see error 4). Second, guard with `if value then` before you index, so an empty object stops the code politely instead of crashing it.
...
<Refs
  items={[
    ...
    { label: "sv_enforceGameBuild convar", href: "https://docs.fivem.net/docs/server-manual/setting-up-a-server-convars/" },
